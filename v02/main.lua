...
-- UI state
g_x,g_y,g_left,g_right,g_tabs,g_focus,g_mousemode=1,1,1,480,{},false,false
g_mx,g_my,g_mb,g_mbp,g_clk=240,135,false,false,false
autonl=true
...
projtyps={
 spark={hitscan=true,objtyp=otyps.spark,dmg=1,cooldown=12},
 firebl={objtyp=otyps.firebl,dmg=4.5,spd=0.25,cooldown=8}
}

stafftyps={{projtyp=projtyps.spark},{projtyp=projtyps.firebl}}
...
function _init()
 ...
 -- Init zbuf/tbuf
 for i=1,480 do
  zbuf[i],tbuf[i]=999,{tile=0,tx=0}
 end
 
 -- Player
 player={x=64,y=64,a=0,spd=0.08,keys={},hp=10,cooldown=0,staff=stafftyps[1],staffi=1}
...
function raycastwalls()
 ...
 for i=0,479 do
  ...
  local dx,dy=240-i,sdist
  -- Rotate ray direction: standard rotation matrix
  -- [ca -sa][dx]   Forward is (0,1) at angle 0
  -- [sa  ca][dy]   X increases right, Y increases down
  dx,dy=ca*dx-sa*dy,sa*dx+ca*dy
  local z,mx,my,tile,tx=raycast(player.x,player.y,dx,dy,sa,ca)
  zbuf[i+1],tbuf[i+1]=z,{tile=tile,tx=tx}
  maxz=max(maxz,z)
...
function draw_3d_view()
 ...
 local y0,y1=max(0,flr(135-h/2)),min(269,ceil(135+h/2))
 
 -- Ceiling (solid color placeholder)
 if y0>0 then
  rectfill(0,0,479,y0,col_ceiling)
 end
 
 -- Floor (solid color placeholder)
 if y1<269 then
  rectfill(0,y1,479,269,col_floor)
 end
 ...
 for x=0,479 do
  ...
  if z<999 and t.tile>0 then
   ...
   local y0,y1=135-h/2,135+h/2
   y0,y1=max(0,flr(y0)),min(269,ceil(y1))
...
function draw_sprites()
 ...
 for ob in all(sobj) do
  ...
  if t then
   ...
   local sx,sy,sw,sh=x*f+240,y*f+135,w*f,(t.h or 1)*f
   local x0,x1,y0,y1=max(0,flr(sx-sw/2)),min(479,ceil(sx+sw/2)),max(0,flr(sy-sh/2)),min(269,ceil(sy+sh/2))
...
function update_player()
 ...
 if btn(5) and player.cooldown==0 then
  spawnproj(player.staff.projtyp,player,player.a)
  player.cooldown=player.staff.projtyp.cooldown
 end
end
...
function trymoveto(pl,nx,ny,radius,opendoors)
 if not iscol(nx,ny,radius,opendoors) then
  pl.x,pl.y=nx,ny
  return true
 end
 if abs(pl.x-nx)>0.01 and not iscol(nx,pl.y,radius,opendoors) then
  pl.x=nx
  return true
 end
 if abs(pl.y-ny)>0.01 and not iscol(pl.x,ny,radius,opendoors) then
  pl.y=ny
  return true
 end
 return false
...
function iscol(px,py,radius,opendoors)
   ...
   if tile>0 then
    if tile==door_normal or tile==door_locked then
     ...
     if door and door.open<1 then
      if opendoors then door.opening=true end
      return true
     end
    else
     return true
 ...
 for e in all(enem) do
  ...
  elseif e.state=="adv" then
   ...
   elseif d>=t.spd then
    ...
    removeobject(e.obj)
    if not iscol(nx,ny,t.crad,t.opendoors) then
     e.obj.pos[1],e.obj.pos[2]=nx,ny
    end
    addobject(e.obj)
   end
  elseif e.state=="att" then
   e.timer-=1
   if e.timer<=0 then
    local dx,dy=px-x,py-y
    if sqrt(dx*dx+dy*dy)<=t.attrng then
     hurtplayer(t.dmg)
    end
    e.state="adv"
   end
...
-- Projectiles
--=======================
function spawnproj(projtyp,owner,ang)
 if projtyp.hitscan then
  local sa,ca=sin(ang),cos(ang)
  local colobj,d=hitscan(owner.x,owner.y,sa,ca)
  if colobj and colobj.enem then
   hurtenemy(colobj.enem,projtyp.dmg)
  end
 else
  local sa,ca=sin(ang),cos(ang)
  local ob=spawnobject(projtyp.objtyp,owner.x+0.3*ca,owner.y+0.3*sa)
  ob.proj={obj=ob,typ=projtyp,owner=owner,xd=ca*projtyp.spd,yd=sa*projtyp.spd}
  add(proj,ob.proj)
 end
end
...
function hitscan(x,y,dx,dy)
 local sa,ca=dx,dy
 local d,hx,hy=raycast(x,y,dx,dy,sa,ca)
 local hitobj
 for gx=flr(min(x,hx)/objgridsize),flr(max(x,hx)/objgridsize) do
  for gy=flr(min(y,hy)/objgridsize),flr(max(y,hy)/objgridsize) do
   if gx>=0 and gx<=27 and gy>=0 and gy<=27 then
    for ob in all(objgrid[gx+1][gy+1]) do
     if ob.pos and ob.typ and ob.typ.solid then
      local dx,dy=ob.pos[1]-x,ob.pos[2]-y
      local dn=dx*ca-dy*sa
      if abs(dn)<=(ob.typ.w or 0.5)*0.5 then
       local dt=dx*sa+dy*ca
...
function update_projectiles()
 for pr in all(proj) do
  local ob,pt=pr.obj,pr.typ
  ob.pos[1],ob.pos[2]=ob.pos[1]+pr.xd,ob.pos[2]+pr.yd
  if iscol(ob.pos[1],ob.pos[2],0.2) then
   removeobject(ob)
   del(proj,pr)
  else
   local hit=false
   for gx=flr(ob.pos[1]/objgridsize)-1,flr(ob.pos[1]/objgridsize)+1 do
    for gy=flr(ob.pos[2]/objgridsize)-1,flr(ob.pos[2]/objgridsize)+1 do
     if gx>=0 and gx<=27 and gy>=0 and gy<=27 then
      for tob in all(objgrid[gx+1][gy+1]) do
       if tob.enem and pr.owner~=tob then
        local dx,dy=tob.pos[1]-ob.pos[1],tob.pos[2]-ob.pos[2]
        if abs(dx)<0.3 and abs(dy)<0.3 then
         hurtenemy(tob.enem,pt.dmg)
         hit=true
        end
       end
      end
...
function generate_dungeon()
 ...
 -- Generate exit
 if #gennodes>0 then
  local exitroom=gennodes[flr(rnd(#gennodes))+1]
  if exitroom then
   local x,y=getwallpt(exitroom.r)
   if isclearwallpt(x,y,exitroom.r) then
    wallgrid[x][y]=exit_tile
   end
  end
 end
...
function trygenerateroom()
 ...
 if door1 then
  if rnd(1)<doorgen then
   makedoor(door1[1],door1[2],door_normal)
  else
 ...
 if door2 and (not door1 or door1[1]~=door2[1] or door1[2]~=door2[2]) then
  if rnd(1)<doorgen then
   makedoor(door2[1],door2[2],door_normal)
  else
...
function isclearspawnpt(x,y,c)
 ...
 for cx=x-c,x+c do
  for cy=y-c,y+c do
   if iscol(cx,cy,0.4) then return false end
  end
 end
 return true
end
...
function draw_minimap()
 local mx,my,mw,mh=360,10,110,110
 clip(mx,my,mw,mh)
 ...
 circfill(mx+55,my+55,2,10)
 -- Direction indicator aligned with movement: forward = (sin(a), cos(a))
 local sa,ca=sin(player.a),cos(player.a)
 line(mx+55,my+55,mx+55+sa*6,my+55+ca*6,10)
...
function g_beg()
 ...
 -- Reset state
 g_x,g_y,g_left,g_right,autonl=1,1,1,480,true
