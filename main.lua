...
-- Expose logger globally so included files can use it
log = require("log")
...
include "src/procgen/door_system.lua"
include "src/procgen/dungeon_gen.lua"
include "src/procgen/dungeon_harness.lua"

local r_state = include "src/render/r_state.lua"
local r_batch = include "src/render/r_batch.lua"
local r_view = include "src/render/r_view.lua"
local r_cast = include "src/render/r_cast.lua"
local r_floor = include "src/render/r_floor.lua"
local r_walls = include "src/render/r_walls.lua"
local r_sprites = include "src/render/r_sprites.lua"
local r_diag = include "src/render/r_diag.lua"
...
local function switch_scene(name, params)
	...
	if not scene then
		log.warn("unknown scene %s", tostring(name))
		return
	end
...
function _init()
	...
	r_batch.init()
	log.info("[main] new renderer initialized")
	...
	-- Permanently disabled for production performance
	...
	log.debug("preloading texture cache...")
	...
	log.debug("texture cache preloaded: %sms", tostring(preload_time))
	...
	-- removed legacy local log() to avoid clashing with global logger

	-- validate all configured sprites exist (comment out for production)
	validate_sprite_configuration()

	log.info("picotron raycast engine v1.0")
	switch_scene("menu")
end
...
function validate_sprite_configuration()
	...
	for enemy in all(enemy_types) do
		if not get_spr(enemy.sprite) then
			log.warn("enemy sprite %s (%s) not found in GFX files", tostring(enemy.sprite), tostring(enemy.name))
		end
	end
	...
	for dec in all(decoration_types) do
		if not get_spr(dec.sprite) then
			log.warn("decoration sprite %s (%s) not found in GFX files", tostring(dec.sprite), tostring(dec.name))
		end
	end
	...
	for texset in all(texsets) do
		for variant in all(texset.variants) do
			if not get_spr(variant) then
				log.warn("wall texture sprite %s not found in GFX files", tostring(variant))
			end
		end
	end
	...
	for typ in all(planetyps) do
		if not get_spr(typ.tex) then
			log.warn("floor/ceiling sprite %s not found in GFX files", tostring(typ.tex))
		end
	end
end
...
function update_gameplay()
	...
	if btnp(12) then
		...
		log.info("Diagnostics logging: %s", tostring(enable_diagnostics_logging))
	end
...
local function draw_lab_map(state)
	local margin_outer = 4
	local map_padding = 3
	local log_panel_width = 176
	local log_panel_margin = 8

	local left_x0 = margin_outer
	local left_x1 = screen_width - log_panel_width - log_panel_margin - 1
	if left_x1 < left_x0 + 32 then left_x1 = left_x0 + 32 end
	local left_width = left_x1 - left_x0 + 1
	local left_top = margin_outer
	local left_bottom = screen_height - margin_outer
	rectfill(left_x0, left_top, left_x1, left_bottom, 0)
	clip(left_x0, left_top, left_width, left_bottom - left_top + 1)

	local min_x = map_size
	local min_y = map_size
	local max_x = -1
	local max_y = -1
	local function extend_bbox(x0, y0, x1, y1)
	...
	for node in all(gen_nodes) do
		...
		extend_bbox(rect[1], rect[2], rect[3], rect[4])
	end
	for edge in all(gen_edges) do
		if edge.b1 then extend_bbox(edge.b1.x, edge.b1.y, edge.b1.x, edge.b1.y) end
		if edge.b2 then extend_bbox(edge.b2.x, edge.b2.y, edge.b2.x, edge.b2.y) end
		if edge.metadata and edge.metadata.anchor then
			local anchor = edge.metadata.anchor
			extend_bbox(anchor.x, anchor.y, anchor.x, anchor.y)
		end
	end
	...
	local bbox_margin = 2
	min_x = max(0, min_x - bbox_margin)
	min_y = max(0, min_y - bbox_margin)
	max_x = min(map_size - 1, max_x + bbox_margin)
	max_y = min(map_size - 1, max_y + bbox_margin)

	local bbox_w = max_x - min_x + 1
	local bbox_h = max_y - min_y + 1

	local available_w = left_width - map_padding * 2
	local available_h = (left_bottom - left_top + 1) - map_padding * 2
	local max_scale = flr(min(available_w / bbox_w, available_h / bbox_h))
	local desired_scale = 2
	local scale = max(1, min(max_scale, desired_scale))
	local map_px_w = bbox_w * scale
	local map_px_h = bbox_h * scale
	local ox = left_x0 + map_padding + flr((available_w - map_px_w) / 2)
	local oy = left_top + map_padding + flr((available_h - map_px_h) / 2)

	rectfill(ox - 2, oy - 2, ox + map_px_w + 1, oy + map_px_h + 1, 1)
	for x = min_x, max_x do
		for y = min_y, max_y do
			...
			local px0 = ox + (x - min_x) * scale
			local py0 = oy + (y - min_y) * scale
			rectfill(px0, py0, px0 + scale - 1, py0 + scale - 1, color)
		end
	end
	for node in all(gen_nodes) do
		local r = node.rect
		rect(ox + (r[1] - min_x) * scale, oy + (r[2] - min_y) * scale, ox + (r[3] - min_x) * scale, oy + (r[4] - min_y) * scale, 11)
	end
	for edge in all(gen_edges) do
		local c = edge.locked and 8 or 12
		local x1 = ox + (edge.n1.midx - min_x) * scale
		local y1 = oy + (edge.n1.midy - min_y) * scale
		local x2 = ox + (edge.n2.midx - min_x) * scale
		local y2 = oy + (edge.n2.midy - min_y) * scale
		line(x1, y1, x2, y2, c)
		if edge.b1 then
			local bx = ox + (edge.b1.x - min_x) * scale
			local by = oy + (edge.b1.y - min_y) * scale
			rectfill(bx, by, bx + scale - 1, by + scale - 1, c)
		end
		if edge.b2 then
			local bx = ox + (edge.b2.x - min_x) * scale
			local by = oy + (edge.b2.y - min_y) * scale
			rectfill(bx, by, bx + scale - 1, by + scale - 1, c)
		end
	end
	for door in all(doors) do
		local c = door.dtype == door_locked and 8 or 10
		local dx = ox + (door.x - min_x) * scale
		local dy = oy + (door.y - min_y) * scale
		rectfill(dx, dy, dx + scale - 1, dy + scale - 1, c)
	end
	for ob in all(objects) do
		if ob.pos then
			local cx = ox + (ob.pos[1] - min_x) * scale
			local cy = oy + (ob.pos[2] - min_y) * scale
			local color = 7
			if ob.typ == obj_types.key then
				color = 9
			elseif ob.typ == obj_types.hostile_npc then
				color = 8
			elseif ob.typ == obj_types.non_hostile_npc then
				color = 13
			elseif ob.typ and ob.typ.kind == "interactable" then
				color = 12
			end
			circfill(cx, cy, max(1, scale \ 2), color)
		end
	end
	clip()
	print("controls: Z=random  X=replay  ←/→ seed  F=harness", left_x0, screen_height - 20, 7)
	print("scroll log: ↑/↓    menu: G or MENU", left_x0, screen_height - 12, 7)
end

local function draw_lab_history(state)
	local log_panel_width = 176
	local log_panel_margin = 8
	local panel_x = screen_width - log_panel_width - log_panel_margin
	local panel_w = log_panel_width
	local panel_top = 12
	...
	clip(panel_x - 4, panel_top - 4, panel_w + 6, panel_h + 6)
	...
	print(seed_text, panel_x, y, 7)
	y += 8
	print(rooms_text, panel_x, y, 7)
	y += 8
	print(locked_text, panel_x, y, 7)
	y += 12
	...
	local max_chars = max(1, flr((panel_w - 12) / 4))
	...
	if max_lines <= 0 then
		...
		if #history == 0 then
			...
			local lines_remaining = max_lines
			local idx = cursor + 1
			while idx <= #history and lines_remaining > 0 do
				local entry = history[idx]
				if entry then
					local pos = 1
					while pos <= #entry and lines_remaining > 0 do
						local chunk = sub(entry, pos, pos + max_chars - 1)
						print(chunk, panel_x, y, 7)
						y += line_h
						lines_remaining -= 1
						pos += max_chars
					end
...
scenes.dungeon_lab = {
	...
	update = function(state)
		...
		elseif btnp(2) or keyp("up") then
			state.history_cursor = max(0, (state.history_cursor or 0) - 1)
		elseif btnp(3) or keyp("down") then
			local history = state.history or {}
			local max_cursor = max(0, #history - 1)
...
function draw_minimap()
	...
	for ob in all(objects) do
		local c = 7
		if ob.typ and ob.typ.kind == "hostile_npc" then
			c = 8
		elseif ob.typ and ob.typ.kind == "direct_pickup" then
			if ob.typ.subtype == "heart" then
				c = 14
			elseif ob.typ.subtype == "key" then
				c = 9
			end
		elseif ob.typ and ob.typ.kind == "interactable" then
			if ob.typ.subtype == "exit" then c = 12 end
		elseif ob.typ and ob.typ.kind == "decorative" then
			c = 13
		end
		local x = ob.pos[1]
		local y = ob.pos[2]
...
function draw_minimap_hud()
	local hud_w = math.ceil(screen_width * 0.25)
	local hud_h = math.ceil(screen_height * 0.25)
	...
	circfill(px, py, 2, 10)
	local ca = cos(player.a)
	local sa = sin(player.a)
	line(px, py, px + ca * 6, py + sa * 6, 10)
...
function collect_item(ob)
	if ob.typ.subtype == "key" and ob.keynum then
		...
		log.info("collected key %s", tostring(ob.keynum))
	elseif ob.typ.subtype == "heart" then
		player.hp = min(100, player.hp + 20)
		log.info("collected heart")
	else
		log.info("collected item")
	end
end
...
function generate_new_floor()
	...
	log.info("floor complete! difficulty: %s", tostring(gen_params.difficulty))

	-- level load diagnostic summary
	log.debug("=== LEVEL LOAD DIAGNOSTICS ===")
	log.debug("Floor: %s", tostring(current_floor or "unknown"))
	log.debug("Difficulty: %s", tostring(gen_params.difficulty))
	log.debug("Rooms: %s", tostring(gen_stats.rooms))
	log.debug("Objects: %s", tostring(gen_stats.objects))
	log.debug("Seed: %s", tostring(gen_stats.seed))
end
